        ...
        <form action="{{ route('suppliers/update', $supplier->id) }}" method="POST">
            ...
            <div class="flex mb-4">
                <div class="w-1/2 pr-2">
                    ...
                    <input type="text" id="name" name="name" required class="border border-gray-300 px-4 py-2 rounded w-full" placeholder="Nome do fornecedor" value="{{ $supplier->name }}" />
                </div>
                <div class="w-1/2 pl-2">
                    <label for="cnpj" class="block text-lg font-medium text-gray-700 mb-4">CNPJ</label>
                    <input type="text" id="cnpj" name="cnpj" required placeholder="CNPJ do fornecedor" class="border border-gray-300 px-4 py-2 rounded w-full @error('cnpj') border-red-500 @enderror" value="{{ old('cnpj', $supplier->cnpj) }}" />
                    @error('cnpj')
            ...
            <div class="flex justify-end mt-16">
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 transition">Atualizar Fornecedor</button>
            </div>
        </form>
